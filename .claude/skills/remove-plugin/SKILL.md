...
2. **Sync docs.** Run `bash .claude/skills/add-plugin/scripts/sync-readme.sh` (regenerates the table;
   shows the placeholder if that was the last plugin) **and**
   `bash .claude/skills/add-plugin/scripts/sync-site.sh` (regenerates the Pages `site/`; gate
   `18-pages-in-sync` fails if it drifts), then add a `CHANGELOG.md` bullet under `## [Unreleased]`
   (e.g. `- Remove \`<name>\` from the marketplace.`).
...
4. **PR.** Branch `chore/remove-<name>`, `git add` the manifest + README + CHANGELOG + `site/`, commit
...
- Touches only `.claude-plugin/marketplace.json`, `README.md`, `CHANGELOG.md`, and the generated
  `site/`; never the plugin's own repo.
...
and the working-tree state of the paths this skill touches:
