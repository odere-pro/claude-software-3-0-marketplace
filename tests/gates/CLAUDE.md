...
| `18-pages-in-sync` | `site/` is byte-identical to what `sync-site.sh` would generate today (byte-equality on index.html/sitemap.xml/robots.txt); no remote-fetch primitives in the generator or output HTML; **real XSS-payload escape check**: runs the generator against a synthetic manifest with injection vectors in every rendered field (including `<svg onload=>` and `<iframe src=javascript:>` — vectors outside the fixed set) and asserts the HTML output: (3a) no live `onerror=` attribute, (3b) no `"><script>` attribute-breakout, (3c) no `</script` inside JSON-LD beyond the closing tag, (3d) no `<script>` tags injected via license/description, **(3e) vector-agnostic scan** — after stripping legitimate script blocks, no raw `<svg`, `<iframe`, `<img`, or `<script` from any payload field survives unescaped (complements the fixed-vector checks: catches novel tag types 3a/3b/3d would miss if html\_esc were bypassed); SEO files present and non-empty; JSON-LD block parses as valid JSON; passes at N=0 (empty plugins). SKIPs if generator/site/ absent or jq missing | CRITICAL |
