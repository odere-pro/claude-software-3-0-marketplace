...
#   - every plugin entry is a `github` source whose repo is odere-pro/<entry-name>,
...
problems="$(jq -r '
  ...
  + [ (.plugins // []) | to_entries[] | .key as $i | .value as $p |
        ( (if (($p.name // "") != "") then empty else "plugins[\($i)].name missing" end),
          ...
          (if (($p.source.repo // "") == ("odere-pro/" + ($p.name // ""))) then empty else "plugins[\($i)].source.repo (\($p.source.repo // "")) must equal odere-pro/\($p.name // "")" end),
